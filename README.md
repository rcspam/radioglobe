...
- Panel or system tray icon with a playing badge, tooltip showing the current station, its local time and title, with previous / play-pause / next / stop / mute buttons and the volume slider, left click to open, middle click for a random station, wheel to change volume, right click for Plasma's widget menu with "Random station" and "Stop and quit mpv"
