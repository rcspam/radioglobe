...
- A filter menu next to the search field sorts the list (popularity, votes, name, bitrate) and keeps only one codec (MP3 or AAC) or a minimum bitrate; it applies to whatever list is loaded, and the globe follows. Right-click a station to vote for it on Radio Browser
