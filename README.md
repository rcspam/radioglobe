...
- Stations come from the [Radio Browser](https://www.radio-browser.info/) community API: world view (up to a configurable cap, default 3000 stations), per-country lists (the 300 most listened plus up to 500 located ones, 1500 for the largest countries, so the globe fills up), and free-text search across name, country and tag
- While a search is active, or a country is open, the globe shows those stations only, so typing a tag such as "jazz" maps where they are; the World tab brings the world back
...
- World / Favorites / Recent tabs, a station list with country, codec and bitrate, and a player bar with previous / play-pause / next / stop / mute / volume / favorite. Favourites can be renamed in place (pencil), recent entries removed (trash); the last 20 stations played are kept
