...
- A filter menu next to the search field sorts the list (popularity, votes, name, bitrate) and keeps only some codecs (the directory's own list, with station counts in the settings) or a minimum bitrate; it applies to whatever list is loaded, the globe follows, and the same choices sit in the General page of the settings. Right-click a station to vote for it on Radio Browser
