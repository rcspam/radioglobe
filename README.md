...
- A rotatable, zoomable globe (Canvas 2D) with one point per live radio station. Drag to spin, scroll or use the + / - buttons to zoom (a glide whose step is a setting, 20 % by default), click a point to play, click a country to browse its stations
...
- A filter menu next to the search field sorts the list (popularity, votes, name, bitrate) and keeps only some codecs (MP3, AAC, OGG; the settings show how many stations each covers) or a minimum bitrate; it applies to whatever list is loaded, the globe follows, and the same choices sit in the General page of the settings. Right-click a station to vote for it on Radio Browser
- While a search is active, or a country is open, the globe shows those stations only, so typing a tag such as "jazz" maps where they are; with a country open the search stays inside it; the cross next to its name, a click on the sea, Escape or the World tab bring the world back
...
- World / Favorites / Recent tabs, a station list with country, codec and bitrate, and a player bar with previous / play-pause / next / stop / mute / volume / favorite. Right-click a station (in the list or in the player) for a menu: play, favorite, copy the stream URL, open its homepage, and Properties with everything Radio Browser knows about it. Favourites can be renamed in place (pencil), recent entries removed (trash); the last 20 stations played are kept. Next / previous walk the list the station was played from, or the favourites (General page)
- At startup the last station played is back in the player, ready to play (General page: it can also start on its own)
- The local time where the station broadcasts sits at the end of the station name (zone picked from the system's `zone1970.tab` by country and nearest reference city; hover it for the zone name and UTC offset). Names and titles too wide for the player or the tooltip scroll in a loop (or back and forth, or are cut: General page)
- Panel or system tray icon with a playing badge, tooltip (opening after a configurable delay, 300 ms by default) showing the current station, its local time and title, with previous / play-pause / next / stop / mute buttons and the volume slider, left click to open, middle click for a random station, wheel to change volume, right click for Plasma's widget menu with "Random station" and "Stop and quit mpv"
